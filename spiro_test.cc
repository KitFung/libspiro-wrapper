...
#include "libspiro/spiro-config.h"     /* for ./configure test settings like VERBOSE */
...
rs_check_vals verify_rs5[] = {
    /* iteration4	verify_rs6 */
...
void load_test_curve(spiro_cp *spiro, int *nextknot, int c) {
  ...
  spiro_cp path3[] = {                 /* this will fail to converge */
                      ...
                      {341, 138, 'o'}, {386, 72, 'o'}, {443, 141, 'o'},
                      {467, 231, 'o'}, {377, 333, '}'}};
  int knot3[] = {2, 1, 9, 1, 1, 0};
  ...
  spiro_cp path5[] = {/* verify curve data with []. */
                      {0, 0, '{'},     {100, 100, 'c'},  {200, 200, '['},
                      {300, 200, ']'}, {400, 150, 'c'},  {300, 100, '['},
                      {200, 100, ']'}, {150, 50, 'c'},   {100, 0, '['},
                      {0, -100, ']'},  {-50, -200, 'c'}, {-80, -250, '}'}};
  int knot5[] = {1, 4, 1, 3, 3, 1, 4, 2, 1, 2, 1, 0};
  spiro_cp path6[] = {/* verify curve data with ah. */
                      {0, 0, '{'},     {100, 100, 'c'}, {200, 200, 'a'},
                      {300, 200, 'h'}, {300, 150, 'c'}, {200, 100, 'a'},
                      {100, 100, 'h'}, {150, 50, 'c'},  {100, 0, 'a'},
                      {0, -100, 'h'},  {50, -100, 'c'}, {20, -150, '}'}};
  int knot6[] = {1, 4, 3, 3, 4, 2, 2, 1, 0, 0, 0, 0};
  spiro_cp path7[] = {
      /* loop stops with ah curves. */
      {0, 200, 'o'},   {50, 210, 'o'}, {100, 0, 'a'},
      {100, -50, 'h'}, {0, -190, 'a'}, {-50, -195, 'h'},
      {-90, 0, 'a'},   {-95, 50, 'h'}, /* call_tests 7 and 8 end here */
      {0, 0, 'z'}                      /* call_test 12 ends with ah z */
  };
  int knot7[] = {1, 1, 5, 5, 6, 3, 0, 0, 0};
  spiro_cp path10[] = {/* start loop with ah curves */
#ifndef TEST_10_11
                       {100, 0, 'a'},    {100, -50, 'h'}, {0, -190, 'a'},
                       {-50, -195, 'h'}, {-90, 0, 'a'},   {-95, 50, 'h'},
                       {0, 200, 'o'},    {50, 210, 'o'}
#else
                       {100, 0, '['},    {100, -50, ']'}, {0, -240, 'a'},
                       {-50, -245, 'h'}, {-90, -50, '['}, {-95, 0, ']'},
                       {-5, 200, 'o'},   {45, 210, 'o'}
#endif
  };
  int knot10[] = {5, 6, 3, 1, 1, 5, 0, 0};
  spiro_cp path13[] = {/* start open curve using {h */
                       {100, 0, '{'},    {100, -50, 'h'}, {0, -190, 'a'},
                       {-50, -195, 'h'}, {-90, 0, 'a'},   {-95, 50, 'h'},
                       {0, 200, 'o'},    {50, 210, '}'}};
  int knot13[] = {5, 6, 3, 1, 1, 5, 0, 0};
...
int test_curve(int c) {
  ...
  /* else if ( c==3 ) rsp = NULL; expecting failure to converge */
    ...
    rsp = verify_rs5; /* same angles used for #6 */
  else if (c == 7)
    rsp = verify_rs7; /* stop curve with ah code */
  else if (c == 8)
    rsp = verify_rs8;
  else if (c == 9)
    rsp = verify_rs9;
  else if (c == 10)
    rsp = verify_rs10; /* start curve using ah. */
  else if (c == 11)
    rsp = verify_rs11;
  else if (c == 12)
    rsp = verify_rs7; /* test #12 uses path7[]. */
  else if (c == 13)
    rsp = verify_rs13; /* almost same as path11 */
  else if (c == 14)
    rsp = verify_rs14; /* very large path9 copy */
  else
    rsp = verify_rs15; /* sub-atomic path9 copy */
  ...
  for (i = j = 0; i < cl[c] - 1; i++, j++) {
    ...
    printf("curve %d line %d t=%c x=%f y=%f bend=%f ch=%f th=%f ", c, j,
           segs[i].ty, segs[i].x, segs[i].y, segs[i].bend_th, segs[i].seg_ch,
           segs[i].seg_th);
...
#endif
#else
#if !defined(DO_CALL_TEST6) && !defined(DO_CALL_TEST7) &&   \
    !defined(DO_CALL_TEST8) && !defined(DO_CALL_TEST10) &&  \
    !defined(DO_CALL_TEST11) && !defined(DO_CALL_TEST12) && \
    !defined(DO_CALL_TEST13)
      ...
      printf(
          "FAIL\nerror found with run_spiro() data. Results are not the "
          "same.\n");
      printf("expected line %d t=%c x=%f y=%f bend=%f ch=%f th=%f\n", j,
             spiro[i].ty, spiro[i].x, spiro[i].y, rsp[i].b, rsp[i].ch,
             rsp[i].th);
      free(segs);
      return -2;
    } else
  ...
  if (spiro[i].ty == '}' || spiro[i].ty == 'z')
    ...
    printf("line %d t=%c x=%f y=%f bend=%f ch=%f th=%f\n", j, segs[i].ty,
           segs[i].x, segs[i].y, segs[i].bend_th, segs[i].seg_ch,
           segs[i].seg_th);

  /* Quick visual check shows X,Y knots match with each pathN[] */
  printf(
      "---\ntesting spiro_to_bpath() using data from "
      "run_spiro(data=path%d[],len=%d).\n",
      c, cl[c]);
  bc = new_bezctx_test();
#if defined(DO_CALL_TEST14) || defined(DO_CALL_TEST15)
...
#if !defined(DO_CALL_TEST4) && !defined(DO_CALL_TEST6) &&   \
    !defined(DO_CALL_TEST7) && !defined(DO_CALL_TEST8) &&   \
    !defined(DO_CALL_TEST9) && !defined(DO_CALL_TEST10) &&  \
    !defined(DO_CALL_TEST11) && !defined(DO_CALL_TEST14) && \
    !defined(DO_CALL_TEST15)
...
#define S_TESTS S_TESTP * 5
  ...
  for (i = 0; i < S_TESTS; i++) {
    ...
    if ((bc[i]->my_curve =
             (my_curve_data *)calloc(S_RESULTS, sizeof(my_curve_data))) == NULL)
      goto test_multi_curves_exit;
  ...
  for (i = 0; i < S_TESTS; i++) {
    ...
    for (j = 0; j < scl[i]; j++) {
      if (i & 1) {
        ...
        temp[j].x *= (i + 1);
        temp[j].x += i * 1000. - 5000.;
        temp[j].y *= (i + 1);
        temp[j].y += i * 3000. - 9000.;
      }
    }
  }
  ...
  for (k = 0; k < S_TESTS;) {
    ...
    for (i = k; i < S_TESTS - 1; i++) {
      ...
      if (pthread_create(&curve_test[i], &tattr, test_a_curve,
                         (void *)&pdata[i])) {
        if (i - k < 20) {
    ...
    for (j = l = 0; j < scl[i] && temp[j].ty != 'z'; j++, l++) {
      if (temp[j].ty == 'h') {
        ...
        printf("  s[%d][ty=%c x=%g y=%g], pk=%d mc[%d][x=%g y=%g]\n", l,
               temp[j].ty, temp[j].x, temp[j].y, pk[j], k, x, y);
#endif
...
test_multi_curves_exit:
  if (nextknot != NULL)
    for (i = 0; i < S_TESTS; i++) free(nextknot[i]);
  if (spiro != NULL)
    for (i = 0; i < S_TESTS; i++) free(spiro[i]);
  free(nextknot);
  free(spiro);
  free(scl);
  if (bc != NULL)
